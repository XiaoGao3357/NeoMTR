...
public class MTRLoggingManager {
    ...
    public String tryGetFriendlyValue(String className, String fieldName, String rawValue, String value, Level world) {
        ...
        if(fieldName.equals("name")) {
            return IGui.formatMTRLanguageName(value);
        }
        ...
        if(fieldName.equals("route_ids")) {
            ...
            for(String routeIdStr : routeIds) {
                ...
                if(route != null) {
                    resultStr.append("\n").append(IGui.formatMTRLanguageName(route.name));
                }
            }
            return resultStr.toString();
        }
        ...
        if(fieldName.equals("platform_ids")) {
            ...
            for(String platformIdStr : platformIds) {
                ...
                String stnName;
                if(station != null) {
                    stnName = IGui.formatMTRLanguageName(station.name);
                } else {
                    stnName = "";
                }
            ...
            for(Map.Entry<String, JsonElement> entry : jsonObject.entrySet()) {
                ...
                if(!exitDestination.isEmpty()) {
                    for(JsonElement jsonElement : exitDestination) {
                        String exitDestinationStr = IGui.formatMTRLanguageName(jsonElement.getAsString());
    ...
    private void processMTREvent(ServerPlayer player, Class<?> dataClass, long id, String name, List<String> oldData, List<String> newData, List<String> oldDataDiff, List<String> newDataDiff, BlockPos[] positions, MTRActionType actionType) {
        ...
        for(String str : oldDataDiff) {
            ...
            String friendlyValue = tryGetFriendlyValue(className, key, rawValue, value, player.level());
        ...
        String displayedName = !name.isEmpty() ? " \\\"" + IGui.formatMTRLanguageName(name) + "\\\" " : " ";
