...
public class RenderNameProjector<T extends BlockRoundelBase.TileEntityBlockRoundelBase> extends BlockEntityRendererMapper<T> {
	...
	public void render(T entity, float tickDelta, PoseStack matrices, MultiBufferSource vertexConsumers, int light, int overlay) {
		...
		if (Config.useMTRFont()) {
			STYLE = Style.EMPTY.withFont(LUAddon.id(font));
		} else {
			STYLE = Style.EMPTY;
		}
		...
		final FormattedCharSequence roundelText = Text.literal(IGui.textOrUntitled(IGui.formatMTRLanguageName(station == null ? "" : station.name)).toUpperCase(Locale.ROOT)).setStyle(STYLE).getVisualOrderText();
